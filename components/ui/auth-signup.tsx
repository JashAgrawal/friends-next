...
export function AuthSignUp({
  ...
  const { addToWatchlist, updateContinueWatching } = useContentStore();
  ...
  const migrateGuestData = async () => {
    try {
      ...
      if (guestData.watchlist.length > 0) {
        guestData.watchlist.forEach(item => {
          addToWatchlist(item);
        });
      }
      
      // Migrate continue watching items
      if (guestData.continueWatching.length > 0) {
        guestData.continueWatching.forEach(item => {
      ...
      toast.success("Your guest data has been migrated to your account");
    } catch (error) {
      console.error("Error migrating guest data:", error);
      toast.error("Failed to migrate your guest data");
    }
  };
  ...
  const handleEmailSignUp = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      // Migrate guest data after successful sign-up
  ...
  const handleProviderSignIn = async (provider: string) => {
    ...
    if (result.success) {
      // For social sign-in, migration will happen after redirect
