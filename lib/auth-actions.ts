...
export async function onUserLogin(params: {
  ...
  // Note: Guest data migration is handled client-side through the useGuestDetection hook
  // This ensures the migration happens after the client-side authentication state is updated
  // and localStorage is accessible. The migration will be triggered automatically when:
  // 1. User is authenticated
  // 2. Guest data is detected in localStorage
  // 3. Migration hasn't been triggered yet
  // 
  // The migration process includes:
  // - Checking for conflicts with existing user data
  // - Migrating watchlist items from localStorage to database
  // - Migrating continue watching items from localStorage to database
  // - Showing user notification about migration results
  // - Clearing guest data from localStorage after successful migration
